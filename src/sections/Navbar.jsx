...
const Navbar = () => {
    ...
    // move toggleMenu to component scope so onClick can access it
    const toggleMenu = () => {
    ...
    useGSAP(() => {
        // ensure we pass actual DOM nodes (spread linksRef.current)
    ...
    return (
        <>
          <nav ref={navRef} className=" fixed z-50 flex flex-col justify-between w-full h-full px-10 
        uppercase bg-black text-white/80 py-28 gap-y-10 md:w-1/2 md:left-1/2 ">


            <div className="flex flex-col text-5xl gap-y-2 
            md:text-6xl lg:text-8xl">
                {["inicio","servicios","about","work","contact"].map(
                    (section, index) => (
          ...
          bg-black rounded-full cursor-pointer w-14 h-14 md:w-20 md:h-20 top-4
          right-10" 
          onClick={toggleMenu}
          style={showBurger
